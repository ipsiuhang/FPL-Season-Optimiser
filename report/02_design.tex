...
The design evolved from the Assignment 4A proposal based on three feedback areas. 
First, inspired by David Bergman's sports analytics work, the project integrates 
three independent datasets covering the full 2024-25 FPL season with actual player statistics and prices, enabling real-world validation against 
historical results.

Second, the system fully models FPL mechanics to enhance realism: automatic 
substitutions accounting for player availability and formation constraints; 
transfer accumulation; and 
buy/sell price differentiation with profit lock for precise budget tracking.

Third, the system implements both Constraint Programming and Mixed Integer Linear 
Programming paradigms for thorough evaluation. MILP solvers include CBC 
\cite{forrest2023cbc} (PuLP default \cite{mitchell2011pulp}), GLPK 
\cite{makhorin2024glpk}, and SCIP \cite{gamrath2025scip}. CP solvers via 
MiniZinc \cite{nethercote2007minizinc} include CP-SAT \cite{perron2025ortools}, 
Chuffed \cite{chu2018chuffed}, and Gecode \cite{schulte2023gecode}, enabling 
direct comparison of approaches and solver efficiency.

The design uses conditional expected points rather than separate stochastic variables for no-shows, 
simplifying the objective. The initially proposed hybrid MILP/CP decomposition 
was dropped as standalone MILP solvers proved sufficiently efficient.

\subsection{Scope}

This FPL optimization system adopts a focused scope balancing feasibility and 
utility. The model optimizes single gameweeks using current state and next-week 
predictions only, with linear objectives matching FPL scoring. Predictions serve 
as external conditional expected points, decoupling optimization from forecasting 
and supporting diverse input sources including statistical models or expert 
judgment.

The system fully implements core FPL elements: squad selection, budget constraints, transfer mechanics, starting lineup formation, automatic substitutions, and captaincy. Data integration merges three sources with conflict resolution and 
duplicate handling. Backtesting simulates the complete 2024-25 season with detailed per-week reports and cumulative metrics.

For tractability, multi-week rolling horizons, FPL chips (Wildcard, Bench Boost, 
Triple Captain, Free Hit), prediction model development, and risk-based or 
non-linear objectives (variance penalties, CVaR, utility curves) are excluded. 
This scope delivers a robust single-gameweek framework without the complexity of 
...
Implementation encountered four primary challenges addressed through targeted 
strategies. Data integration across three sources required reconciliation of 
schema inconsistencies via 
fuzzy matching and manual verification, with temporal alignment ensuring gameweek 
correspondence and completeness checks prioritizing primary sources.

Data quality issues demanded systematic cleaning: missing values were imputed, 
duplicates from mid-season transfers underwent timeline-preserving deduplication, 
inconsistent formats were standardized.

Constraint Programming model development in MiniZinc required adapting to its 
declarative paradigm, managing Python API type conversions for data passing, 
handling complex set operations for constraints like club affiliations with 
intertwined membership and cardinality rules, while limited debugging tools 
complicated tracing unsatisfiable constraints.

CP solver performance proved problematic: timeouts frequently exceeded minutes 
even for modest instances, scalability suffered with larger player pools, and 
solution times varied unpredictably. 
