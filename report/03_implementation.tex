...
The mapped dataset included extraneous variables, so we retained only those vital to the optimization model: \texttt{player\_name}, \texttt{status}, \texttt{chance\_of\_playing\_next\_round}, \texttt{chance\_of\_playing\_this\_round}, \texttt{now\_cost}, \texttt{event\_points} (gameweek-specific FPL points), \texttt{ep\_next} and \texttt{ep\_this} (expected points), and \texttt{gw}.
...
Prior to integration, we assessed data quality by scanning for duplicate (gameweek, player name) pairs, anomalies given the standard one-match-per-club schedule.
...
Prior to aggregation, we tested consistency for duplicated players: the summed \texttt{total\_points} from VAA's two matches should equal OLB's \texttt{event\_points}.
...
This per-record ensemble selected \texttt{ep\_this} in 25,487 cases (93.63\%) and \texttt{xP} in 1,735 (6.37\%), varying by gameweek.

Note: This retrospective method suits backtesting but not prospective use, where outcomes are unknown. It mitigates \texttt{xP} zeros and outliers, outperforming averages or extrema skewed thereby.
...
A fundamental implementation invariant requires each DataFrame to encompass precisely one gameweek, devoid of duplicate \texttt{player\_id} entries. This is enforced via validation at the entry points of all optimization modules: \texttt{pre\_gw1\_step1.py}, \texttt{pre\_gw1\_step2.py}, and \texttt{post\_gw1\_step1.py}.

\begin{lstlisting}[language=Python, caption={Single-Gameweek Validation}]
...
The output comprises a standardized JSON file, wherein each gameweek encompasses 12 keys: eight binary decision dictionaries (\texttt{x}, \texttt{y}, \texttt{c}, \texttt{v}, \texttt{b1}, \texttt{b2}, \texttt{b3}, \texttt{b4})—each mapping all 638 player IDs to \{0,1\}—plus four state variables (\texttt{y0} dict, \texttt{p0} dict, \texttt{B\_bank} scalar, \texttt{f} scalar). This uniform structure promotes data consistency across gameweeks and facilitates integration with backtesting and analytical tools.
...
The primary rationale for a parallel CP formulation alongside MILP is to evaluate alternative modeling paradigms and corroborate solution integrity across independent solver frameworks. MiniZinc's declarative syntax enables set-based representations and global constraints, while CP solvers emphasize constraint propagation over MILP's linear relaxations.

The CP implementation mirrors the MILP's modular architecture, comprising three optimization modules for Pre-GW1 Step 1, Pre-GW1 Step 2, and Post-GW1 phases. Each module integrates a Python wrapper (\texttt{.py}) for data conversion and solver interfacing with a MiniZinc model (\texttt{.mzn}) for declarative constraint formulation. This bifurcation permits MiniZinc's domain-specific syntax to encode optimization logic, while Python oversees data preparation and result extraction.
...
The \texttt{disjoint()} constraint guarantees that no player appears in multiple position sets, while \texttt{array\_union()} reconstructs the complete squad and starting XI from their position-based partitions.
...
The Python data preparation module (\texttt{data\_prep.py}) manages these scalings; post-solving, objective values are rescaled to original units for analysis.
...
Implementation comprises three modules: \texttt{fpl\_validator.py} for rule checks, \texttt{fpl\_point\_calculator.py} for substitutions and scoring, and \texttt{backtester.py} for orchestrating the full season. This modularity decouples validation from computation, supporting isolated testing and cross-optimizer reuse.
...
\paragraph{Transfer Detection}

Transfers are identified by comparing consecutive gameweek squads: bought players appear in the current squad but not the previous, while sold players appear in the previous but not the current. Set difference operations facilitate this detection efficiently.
...
The backtester joins optimizer decisions (via player IDs) with actual performance data from \texttt{cleaned\_data.csv}, where ID mismatches would precipitate lookup failures. It relies on the uniform \texttt{player\_id} scheme from data cleaning, assigning identical IDs to all 638 players across outputs and the dataset. This permits direct DataFrame filtering for gameweek data, followed by squad-specific extraction from decisions. For external users, adherence to this 638-player mapping is requisite; alternatively, forking the repository and implementing extensions is advised to accommodate custom datasets.
